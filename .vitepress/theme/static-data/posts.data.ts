...
const postLoader = createContentLoader(postPattern, {
    excerpt: '<!-- more -->',
    // excerpt: true,
    transform: (data) => data.filter((value) => {
        ...
        title: stringOrUndefined(value.frontmatter.title) ?? throwError(`Post ${value.url} does not contain 'title' in frontmatter.`),
        date: dateOrUndefined(value.frontmatter.date) ?? throwError(`Post ${value.url} does not contain 'date' in frontmatter`),
